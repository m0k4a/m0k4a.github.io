...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (headerTitle) {
        typeEffect(headerTitle, "Moksha", 200);
    }
...
function startCounter() {
    ...
    // ✅ زيادة العداد كل 30 مللي ثانية
    const interval = setInterval(() => {
        if (count <= 100) {
            counterElement.textContent = `${count}%`;
            count++;
        } else {
            clearInterval(interval); // إيقاف العداد عند الوصول إلى 100
            loadingContainer.style.display = "none"; // إخفاء العداد
        }
    }, 5); // سرعة العداد (يمكن تعديلها)
}
